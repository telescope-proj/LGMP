...
LGMP_STATUS lgmpShmClientInit(void * mem, const size_t size, PLGMPClient * result)
{
  ...
  *result = calloc(1, sizeof(**result));
  ...
  struct LGMPShmClient * shm = calloc(1, sizeof(*shm));
  if (!shm)
  {
    free(*result);
    *result = NULL;
  ...
  client->iface    = &lgmpShmClientInterface;
  client->internal = shm;
  shm->mem         = (uint8_t*)mem;
  shm->header      = header;
  shm->hosttime    = atomic_load_explicit(&header->timestamp,
      memory_order_relaxed);
  return LGMP_OK;
}

static void lgmpShmClientFree(PLGMPClient * client)
{
  assert(client);
  if (!*client)
    return;
...
static LGMP_STATUS lgmpShmClientSessionInit(PLGMPClient client,
    uint32_t * udataSize, uint8_t ** udata, uint32_t * clientID)
{
  assert(client);
  struct LGMPShmClient * shm = client->internal;
  struct LGMPHeader * header = shm->header;
  ...
  shm->hosttime         = timestamp;
  shm->lastHeartbeat    = lgmpGetClockMS();
...
static bool lgmpShmClientSessionValid(PLGMPClient client)
{
  assert(client);
  struct LGMPShmClient * shm = client->internal;

  // check if the host has been restarted
  if (unlikely(client->sessionID != shm->header->sessionID))
    return false;

#ifndef LGMP_REALACY

  // check if the heartbeat changed
  const uint64_t hosttime = atomic_load_explicit(&shm->header->timestamp,
      memory_order_relaxed);
  const uint64_t now = lgmpGetClockMS();
  if (likely(shm->hosttime != hosttime))
  {
    shm->lastHeartbeat = now;
    shm->hosttime      = hosttime;
    return true;
  }

  // check if the heartbeat timeout has been exceeded
  if (unlikely(now - shm->lastHeartbeat > LGMP_HEARTBEAT_TIMEOUT))
    return false;
...
static LGMP_STATUS lgmpShmClientSubscribe(PLGMPClient client, uint32_t queueID,
    PLGMPClientQueue * result)
{
  ...
  *result = NULL;
  ...
  uint32_t queueIndex;
  for(queueIndex = 0; queueIndex < shm->header->numQueues; ++queueIndex)
    if (shm->header->queues[queueIndex].queueID == queueID)
    {
      hq = &shm->header->queues[queueIndex];
      break;
    }
  ...
  *result = &client->queues[queueIndex];
  ...
  struct LGMPShmClientQueue * sq = &shm->queues[queueIndex];
  sq->header  = shm->header;
  sq->hq      = hq;

  q->ops      = &lgmpShmClientQueueOps;
  q->internal = sq;
  q->client   = client;
  q->index    = queueIndex;
  q->id       = id;
...
static LGMP_STATUS lgmpShmClientUnsubscribe(PLGMPClientQueue * result)
{
  assert(result);
  ...
  struct LGMPShmClientQueue * sq = queue->internal;
...
static LGMP_STATUS lgmpShmClientAdvanceToLast(PLGMPClientQueue queue)
{
  assert(queue);

  struct LGMPShmClientQueue * sq = queue->internal;
  struct LGMPShmClient * shm = queue->client->internal;
  struct LGMPHeaderQueue *hq = sq->hq;
  const uint32_t bit = 1U << queue->id;
  ...
  if (likely(locked))
  {
    ...
    uint64_t oldTimeout = atomic_load_explicit(&hq->msgTimeout,
        memory_order_relaxed);
    uint64_t newTimeout = atomic_load_explicit(&sq->header->timestamp,
...
static LGMP_STATUS lgmpShmClientProcess(PLGMPClientQueue queue,
    PLGMPMessage result)
{
  assert(queue);
  assert(result);
...
static LGMP_STATUS lgmpShmClientMessageDone(PLGMPClientQueue queue)
{
  assert(queue);
...
static LGMP_STATUS lgmpShmClientSendData(PLGMPClientQueue queue,
...
static LGMP_STATUS lgmpShmClientGetSerial(PLGMPClientQueue queue,
    uint32_t * serial)
{
  struct LGMPShmClientQueue * sq = queue->internal;
  struct LGMPHeaderQueue *hq = sq->hq;
...
const struct LGMPClientInterface lgmpShmClientInterface =
{
  ...
  .free         = lgmpShmClientFree,
  .sessionInit  = lgmpShmClientSessionInit,
  .sessionValid = lgmpShmClientSessionValid,
  .subscribe    = lgmpShmClientSubscribe,
  .unsubscribe  = lgmpShmClientUnsubscribe,
};

const struct LGMPClientQueueOps lgmpShmClientQueueOps =
{
  .advanceToLast = lgmpShmClientAdvanceToLast,
  .process       = lgmpShmClientProcess,
